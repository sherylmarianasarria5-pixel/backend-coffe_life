...
export default class RecomendacionesController {
  ...
  async show({ params, response }: HttpContext) {
    const recomendacion = await Recomendacione.query()
      ...
      .preload('experto')
      .firstOrFail()
    return response.ok({ message: 'Recomendacion encontrada', data: recomendacion })
  }
  ...
  async update({ params, request, response }: HttpContext) {
    ...
    return response.ok({ message: 'Recomendacion actualizada correctamente', data: recomendacion })
  }

  async destroy({ params, response }: HttpContext) {
    const recomendacion = await Recomendacione.findOrFail(params.id)
    await recomendacion.delete()
    return response.ok({ message: 'Recomendacion eliminada correctamente' })
  }
}